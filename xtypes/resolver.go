...
type declInfo struct {
	file      *types.Scope  // scope of file containing this declaration
	lhs       []*types.Var  // lhs of n:1 variable declarations, or nil
	...
	deps map[types.Object]bool // lazily initialized
}
...
func (check *Checker) declarePkgObj(ident *ast.Ident, obj types.Object, d *declInfo) {
	...
	check.declare(Package_scope(check.pkg), ident, obj, token.NoPos)
	check.objMap[obj] = d
	Object_setOrder(obj, uint32(len(check.objMap)))
}
...
func (check *Checker) importPackage(at positioner, path, dir string) *types.Package {
	...
	if path == "C" && (check.conf.FakeImportC || check.conf.go115UsesCgo) {
		imp = types.NewPackage("C", "C")
		Package_setFake(imp, true) // package scope is not populated
		Package_setCgo(imp, check.conf.go115UsesCgo)
	} else {
		// ordinary import
		var err error
		if importer := check.conf.Importer; importer == nil {
			err = fmt.Errorf("Config.Importer not installed")
		} else if importerFrom, ok := importer.(types.ImporterFrom); ok {
		...
		if err != nil {
			...
			if imp == nil {
				...
				imp = types.NewPackage(path, name)
			}
			// continue to use the package as best as we can
			Package_setFake(imp, true) // avoid follow-up lookup failures
		}
	}

	// package should be complete or marked fake, but be cautious
	if imp.Complete() || Package_fake(imp) {
...
func (check *Checker) collectObjects() {
	...
	for fileNo, file := range check.files {
		...
		fileScope := types.NewScope(Package_scope(pkg), pos, end, check.filename(fileNo))
		...
		check.walkDecls(file.Decls, func(d decl) {
			...
			case importDecl:
				...
				if !pkgImports[imp] {
					pkgImports[imp] = true
					Package_appendImport(pkg, imp)
				}

				pkgName := types.NewPkgName(d.spec.Pos(), pkg, name, imp)
				if d.spec.Name != nil {
				...
				if name == "." {
					...
					if check.dotImportMap == nil {
						check.dotImportMap = make(map[dotImportKey]*types.PkgName)
					}
					// merge imported scope with file scope
					for name, obj := range Scope_elems(Package_scope(imp)) {
						...
						if token.IsExported(name) {
							...
							if alt := fileScope.Lookup(name); alt != nil {
								...
								Scope_insert(fileScope, name, obj)
			...
			case constDecl:
				...
				for i, name := range d.spec.Names {
					obj := types.NewConst(name.Pos(), pkg, name.Name, nil, constant.MakeInt64(int64(d.iota)))
			...
			case varDecl:
				lhs := make([]*types.Var, len(d.spec.Names))
				...
				for i, name := range d.spec.Names {
					obj := types.NewVar(name.Pos(), pkg, name.Name, nil)
					lhs[i] = obj
			...
			case typeDecl:
				obj := types.NewTypeName(d.spec.Name.Pos(), pkg, d.spec.Name.Name, nil)
				check.declarePkgObj(d.spec.Name, obj, &declInfo{file: fileScope, tdecl: d.spec})
			case funcDecl:
				name := d.decl.Name.Name
				obj := types.NewFunc(d.decl.Name.Pos(), pkg, name, nil)
				if d.decl.Recv.NumFields() == 0 {
					...
					if name == "init" {
						...
						Func_setParent(obj, Package_scope(pkg))
						...
						check.declare(Package_scope(pkg), d.decl.Name, obj, token.NoPos)
					}
				} else {
					// method
	...
	for _, scope := range fileScopes {
		for name, obj := range Scope_elems(scope) {
			if alt := Package_scope(pkg).Lookup(name); alt != nil {
				obj = Resolve(name, obj) // see types.resolve
	...
	check.methods = make(map[*types.TypeName][]*types.Func)
	for i := range methods {
		m := &methods[i]
		// Determine the receiver base type and associate m with it.
		ptr, base := check.resolveBaseTypeName(m.ptr, m.recv, fileScopes)
		if base != nil {
			Func_setHasPtrRecv_(m.obj, ptr)
...
func (check *Checker) resolveBaseTypeName(seenPtr bool, typ ast.Expr, fileScopes []*types.Scope) (ptr bool, base *types.TypeName) {
	...
	// we're done.
	ptr = seenPtr
	var seen map[*types.TypeName]bool
	for {
		...
		case *ast.SelectorExpr:
			...
			if ident, _ := typ.X.(*ast.Ident); ident != nil && ident.Name == "C" {
				...
				if pname, _ := obj.(*types.PkgName); pname != nil {
					if Package_cgo(pname.Imported()) { // only set if Config.go115UsesCgo is set
		...
		obj := Package_scope(check.pkg).Lookup(name)
		if obj == nil {
			return false, nil
		}
...
func (check *Checker) packageObjects() {
	...
	objList := make([]types.Object, len(check.objMap))
	i := 0
	...
	for _, obj := range objList {
		if obj, _ := obj.(*types.TypeName); obj != nil && obj.Type() != nil {
	...
	var aliasList []*types.TypeName
	var othersList []types.Object // everything that's not a type
...
type inSourceOrder []types.Object

func (a inSourceOrder) Len() int           { return len(a) }
func (a inSourceOrder) Less(i, j int) bool { return Object_order(a[i]) < Object_order(a[j]) }
